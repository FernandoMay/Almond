...
0.2.0 & 2026-09-21 & Added peak coverage, overtime economics, and objective weights, commit \texttt{4831485}.\\
\bottomrule
\end{tabular}
\end{table}}
...
\callout{\textbf{Current answer.} The reproducible demo reaches 100\% modeled and peak coverage with an optimized cost of 16,402 MXN. Its generated current schedule costs 15,680 MXN, covers 74.27\% overall and 98.08\% at peak, so the honest cost delta is \textbf{-722 MXN avoided} or \textbf{-4.6\% savings}. The result is not presented as an artificial 8\% saving: the optimized model buys additional coverage.}
...
Peak coverage & 98.08\% & 100.00\%\\
Understaffing & 62 & 0\\
Peak understaffing & 2 & 0\\
Peak coverage violations & 2 & 0\\
Overstaffing & 45 & 0\\
...
The flow is intentionally explicit: generate one scenario; construct the rotating current schedule; optimize the same scenario; independently analyze and verify the optimized schedule; then compare schedule-derived costs. The CLI also reports objective weights $(1,10000,20000)$, overtime threshold 40, multiplier 1.5, and zero regular-schedule overtime for both current and optimized demo schedules. The negative result is honest. The current schedule spends less because it leaves demand uncovered, while the optimizer spends 722 MXN more to reach complete modeled coverage. Reporting an invented 8\% saving would confuse a cost reduction with a service-level improvement and would violate the project's requirement that savings be computed from assignments and rates.
...
The repository test suite contains sixteen tests, and the observed result for this work unit is:
\begin{verbatim}
.venv/bin/pytest -q
................                                                         [100%]
\end{verbatim}
...
pdflatex -interaction=nonstopmode -halt-on-error \
  -output-directory /tmp/almond-latex docs/ALMOND_TECHNICAL_REPORT.tex
\end{verbatim}
The second LaTeX pass resolves references. The expected PDF is \texttt{/tmp/almond-latex/ALMOND\_TECHNICAL\_REPORT.pdf}. This report documents commit \texttt{4831485}; later code changes require re-running both the demo and the report's metric claims.
...
Exact implementation revision & Git commit \texttt{4831485}.\\
\bottomrule
\end{longtable}
...
\begin{enumerate}
  \item \texttt{README.md}, Almond repository, commit \texttt{4831485}.
  \item \texttt{docs/ARCHITECTURE.md}, Almond repository, commit \texttt{4831485}.
  \item \texttt{docs/SOLVER\_MODEL.md}, Almond repository, commit \texttt{4831485}.
  \item \texttt{docs/DATA\_MODEL.md} and \texttt{docs/ASSUMPTIONS.md}, Almond repository, commit \texttt{4831485}.
  \item \texttt{docs/AI\_ENGINEERING\_LOG.md}, Almond repository, commit \texttt{4831485}.
  \item \texttt{src/almond/} and \texttt{tests/test\_engine.py}, Almond repository, commit \texttt{4831485}.
\end{enumerate}
