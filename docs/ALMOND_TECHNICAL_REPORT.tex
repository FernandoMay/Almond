...
The complete JSON request is the reproducibility input. Repeating it with the fixed single-worker, seed-40 CP-SAT configuration produces byte/semantic-equivalent JSON. The \texttt{almond-api} console script starts Uvicorn on the local development address. No UI, persistence, file upload, authentication, or deployment has been implemented or fabricated.
...
Exact implementation revision & Git commit \texttt{bf18756}.\\
\bottomrule
\end{longtable}
...
\begin{enumerate}
  \item \texttt{README.md}, Almond repository, commit \texttt{bf18756}.
  \item \texttt{docs/ARCHITECTURE.md}, Almond repository, commit \texttt{bf18756}.
  \item \texttt{docs/SOLVER\_MODEL.md}, Almond repository, commit \texttt{bf18756}.
  \item \texttt{docs/DATA\_MODEL.md} and \texttt{docs/ASSUMPTIONS.md}, Almond repository, commit \texttt{bf18756}.
  \item \texttt{docs/AI\_ENGINEERING\_LOG.md}, Almond repository, commit \texttt{bf18756}.
  \item \texttt{src/almond/} and \texttt{tests/test\_engine.py}, Almond repository, commit \texttt{bf18756}.
\end{enumerate}
