...
\texttt{api.py} & Exposes typed FastAPI health and versioned demo boundaries by calling the CLI composition.\\
\bottomrule
\end{longtable}
...
\end{itemize}
The API calls \texttt{cli.run\_demo(seed)} for both demo routes; solver, verification, and economics logic is not duplicated. Pydantic request and response models define the boundary, and invalid request values produce normal FastAPI 422 responses. The \texttt{almond-api} console script starts Uvicorn on the local development address. No UI, persistence, authentication, or deployment has been implemented or fabricated.
...
The repository test suite contains twenty-four tests, and the observed result for this work unit is:
\begin{verbatim}
.venv/bin/pytest -q
........................                                                 [100%]
\end{verbatim}
...
API contract & \texttt{api.py}; \texttt{tests/test\_api.py} & Product boundary & Implemented & The verified core would lack a stable health and demo surface.\\
\bottomrule
\end{longtable}

\section{AI-assisted engineering log}
The project used bounded AI-agent work units with human review between them. One implementation writer created the initial vertical slice; an independent verifier inspected the result; a correction writer fixed the confirmed overlap defect; and later implementation writers added baseline economics and this peak/overtime/objective work unit. The latest unit adds deterministic peak marking, hard-first peak coverage with visible fallback, configurable objective coefficients, and overtime pricing without changing the hard 40-hour optimizer cap.
...
pdflatex -interaction=nonstopmode -halt-on-error \
  -output-directory /tmp/almond-latex docs/ALMOND_TECHNICAL_REPORT.tex
\end{verbatim}
The second LaTeX pass resolves references. The expected PDF is \texttt{/tmp/almond-latex/ALMOND\_TECHNICAL\_REPORT.pdf}. This report documents commit \texttt{efa0500}; later code changes require re-running both the demo and the report's metric claims.
...
Exact implementation revision & Git commit \texttt{efa0500}.\\
\bottomrule
\end{longtable}
...
\begin{enumerate}
  \item \texttt{README.md}, Almond repository, commit \texttt{efa0500}.
  \item \texttt{docs/ARCHITECTURE.md}, Almond repository, commit \texttt{efa0500}.
  \item \texttt{docs/SOLVER\_MODEL.md}, Almond repository, commit \texttt{efa0500}.
  \item \texttt{docs/DATA\_MODEL.md} and \texttt{docs/ASSUMPTIONS.md}, Almond repository, commit \texttt{efa0500}.
  \item \texttt{docs/AI\_ENGINEERING\_LOG.md}, Almond repository, commit \texttt{efa0500}.
  \item \texttt{src/almond/} and \texttt{tests/test\_engine.py}, Almond repository, commit \texttt{efa0500}.
\end{enumerate}
